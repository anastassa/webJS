...
task0.onclick = function(){
                    ...
                    yName = prompt('Привет! Как Вас зовут?');
                    while(yName == null) yName = prompt("Введите имя, пожалуйста")
                    if(yName != null) alert('Добро пожаловать, ' + yName);
                    }
...
bnt.onclick = function(){
    ...
    document.getElementById('data').innerHTML = "Привет, " + yName + "<br \/>" +"Текущая дата: " + now;
}
...
task1.onclick = function (){
    ...
    let s = (a*h)/2;
        ...
        if(!isNaN(s)) alert("Площадь треугольника: " + s);
...
if(isNaN(Math.max(...arr)) || isNaN(Math.min(...arr))) alert("Нужно ввести числа!");
else alert("Максимальный элемент: " + Math.max(...arr) + "\n" + "Минимальный элемент: " + Math.min(...arr));
}
